...
class roomController {
  ...
  async showRoomId(req, res){
    ...
    try{
      ...
      res.status(500).json({ erro: "Erro ao buscar esse quarto em específico"})
    }
  }
  ...
  async updateRoom(req, res){
    ...
    try {
      ...
      if (roomEdited.affectedRows == 0){
        return res.status(404).json({ erro: 'Quarto não encontrado'});  
      }
        res.status(200).json({ mensagem: 'Quarto atualizado com sucesso', atualizacao })
